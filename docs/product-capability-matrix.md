...
| Board / Plan surface | Shipped | Let operators inspect and act on project work from the primary planning view. | SPA routes for `/board`, board subroutes, work-item APIs. | Board exists, but route consolidation still appears to be evolving. | Keep route names aligned with `docs/ui-spec.md`. |
| Refine / Backlog surface | Shipped | Let operators clean up backlog, improve work items, and refine planning state. | `/refine`, `/backlog` redirect, work-item patch routes. | Some old naming still exists through redirects. | Decide whether Backlog is legacy vocabulary or a supported alias. |
