...
| Beads-only mode | Shipped | Run Gemba as a Beads viewer/manager without project or orchestration setup. | `--beads-only`, `--beads-url`, `--beads-history`, Flat/Cascade board views, shared order selector, Status Beads health, RHP Beads history, Graph availability, create/edit/delete routes. | Dolt URL sources remain read-only until a write path exists; source selection from Help is still deterministic setup rather than LLM coaching. | Keep Beads-only docs and hidden-route gating aligned as new Beads surfaces ship. |
