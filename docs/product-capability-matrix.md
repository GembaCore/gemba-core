...
| Board / Plan surface | Shipped | Let operators inspect and act on project work from the primary planning view. | SPA routes for `/board`, board subroutes, work-item APIs, three-lane execution kanban, card status/attention pills. | Board exists, with list/power and legacy deep-link aliases still present. | Keep route names and card-state vocabulary aligned with `docs/ui-spec.md`. |
