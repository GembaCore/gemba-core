...
// Transport names the wire protocol an adaptor speaks. Every adaptor
// declares exactly one transport on its manifest; gm-e3.4 layers
// registration + version negotiation on top.
type Transport string

const (
	// TransportAPI — JSON over HTTP (Gemba's native REST surface).
	TransportAPI Transport = "api"
	// TransportJSONL — newline-delimited JSON over a subprocess' stdio.
	TransportJSONL Transport = "jsonl"
	// TransportMCP — Model Context Protocol (gm-root DD-8). The
	// recommended transport for new OrchestrationPlane adaptors but
