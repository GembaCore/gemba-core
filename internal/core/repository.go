...
func NewRepositoryRegistry() *RepositoryRegistry {
	return &RepositoryRegistry{repos: make(map[RepositoryID]*Repository)}
}

// LoadRepositoryRegistry constructs a RepositoryRegistry from every
// repository TOML file under dir.
func LoadRepositoryRegistry(dir string) (*RepositoryRegistry, error) {
	repos, err := LoadRepositoriesDir(dir)
	if err != nil {
		return nil, err
	}
	return &RepositoryRegistry{repos: repos}, nil
}

// Register adds r to the registry. Returns an error when r is nil,
// r.ID is empty, r.Validate fails, or another repo is already
// registered under r.ID. Re-registering the same pointer is a no-op
// so init paths can be defensive without erroring out.
