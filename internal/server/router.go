...
type Router struct {
	...
	// surfaces). Optional — nil disables audit emission, which is
	// the right behaviour for unit tests that don't care about the
	// audit trail. AttachAuditor wires in production logger.
	...
	// is passed). Stays a hook (rather than a hard dependency on a
	// workspace store) so the workspace catalogue can land in a
	// later bead without rewriting this surface. Nil ⇒ assume 0.
