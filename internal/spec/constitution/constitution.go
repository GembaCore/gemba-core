...
type Constitution struct {
	...
	// Spec-quality knobs (gm-v0sp.7 expansion). All default-off; all
	// inherit SpecStrict when unset.
	RequireDecisionParent *bool
	ForbidOrphanBeads     *bool
	RequirePriority       *bool

	// MinACCount: every Story section must have >=N items in its AC: list.
	// 0 means disabled. Set explicitly; does not inherit SpecStrict.
	MinACCount *int
}
