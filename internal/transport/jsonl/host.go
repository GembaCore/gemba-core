...
// Host is the newline-delimited-JSON-over-stdio adaptor host. It accepts
// in-process adaptors that declare Transport=jsonl; the stdio pump (which
// reads a stream of JSON-RPC-ish frames and dispatches into the bound
// adaptor) lands with gm-e3.5 + the conformance harness.
//
// For gm-e3.4 the job is registration + version negotiation: any adaptor
// declaring TransportJSONL must be bindable here with a clear error when
...
// New returns an empty jsonl Host.
func New() *Host { return &Host{} }

// Transport reports the wire this host serves.
func (h *Host) Transport() core.Transport { return core.TransportJSONL }
...
func (h *Host) RegisterWorkPlane(ctx context.Context, a core.WorkPlane) (transport.Registration, error) {
	if a == nil {
		return transport.Registration{}, fmt.Errorf("jsonl: WorkPlane is nil")
	}
	m, err := a.Describe(ctx)
	if err != nil {
		return transport.Registration{}, fmt.Errorf("jsonl: describe: %w", err)
	}
	if err := m.Validate(); err != nil {
		return transport.Registration{}, fmt.Errorf("jsonl: manifest: %w", err)
	}
...
func (h *Host) RegisterOrchestrationPlane(_ context.Context, a core.OrchestrationPlaneAdaptor) (transport.Registration, error) {
	if a == nil {
		return transport.Registration{}, fmt.Errorf("jsonl: OrchestrationPlaneAdaptor is nil")
	}
	m := a.Describe()
